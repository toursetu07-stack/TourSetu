...
window.showPackageForm = function(pEncoded = null) {
    ...
    area.innerHTML = `
        <div style="background:white; padding:30px; border-radius:15px; border:2px solid #ff9f43; max-width:750px; margin:20px auto; box-shadow:0 10px 30px rgba(0,0,0,0.1);">
            <h2 style="color:#ff9f43; margin:0 0 20px 0;">${isEdit ? '✏️ Edit Package' : '🎒 Create New Package'}</h2>
...
window.handleAuth = async function() {
    ...
    try {
        if (isLoginMode) {
            ...
            if (error) throw error;
            status.innerHTML = `<div style="color:green; padding:10px;">✉️ Check your Inbox! Verification sent to ${email}.</div>`;
        }
    } catch (err) {
        ...
        <div style="display:flex; min-height:100vh; background:#f8f9fa; margin:-20px; font-family:'Inter', sans-serif;">
            <div style="width:260px; background:#2d3436; color:white; padding:25px; position:relative; flex-shrink:0;">
               ...
               <nav>
                   ...
                   <div onclick="window.confirmLogout()" style="padding:15px; cursor:pointer; color:#ff7675; margin-top:50px; font-weight:bold; border-top:1px solid #444;">門 Logout</div>
               </nav>
            </div>
    ...
    container.innerHTML = pkgs.map(p => `
        <div class="card" style="background:white; padding:20px; border-radius:12px; margin-bottom:20px; box-shadow: 0 4px 6px rgba(0,0,0,0.05);">
            <div style="display:flex; justify-content:space-between; align-items:center;">
                ...
                <button onclick="window.showPackageForm('${encodeURIComponent(JSON.stringify(p))}')" style="background:#3498db; color:white; border:none; padding:8px 15px; border-radius:5px; cursor:pointer;">✏️ Edit Details</button>
            </div>
        </div>
    `).join('');
};
