    ...
    if (tabName === 'earnings') {
        ...
        document.getElementById('pkg-list-container').innerHTML = (myPackages || []).map(p => {
             ...
             return `<div style="background:white; padding:20px; border-radius:12px; margin-bottom:15px; display:flex; justify-content:space-between; align-items:center; box-shadow:0 2px 8px rgba(0,0,0,0.05); border-left:5px solid #ff9f43;">
                ...
                <button onclick="handleEditPackageClick('${encoded}')" style="background:#ff9f43; color:white; border:none; padding:10px 22px; border-radius:8px; cursor:pointer; font-weight:bold;">✏️ Edit</button>
            </div>`;
        }).join('');
    }
...
// --- FIX INTERCEPTOR FOR EDIT MODE AND ID MISMATCH ---
window.handleEditPackageClick = function(encodedData) {
    const packageData = JSON.parse(decodeURIComponent(encodedData));
    
    // 1. पहले फॉर्म को रेंडर करने के लिए मूल फ़ंक्शन को कॉल करें
    if (typeof window.renderPackageForm === 'function') {
        window.renderPackageForm(packageData);
    }
    
    // 2. तुरंत बाद एक छोटा सा टाइमआउट लगायें ताकि HTML लोड हो सके और वैल्यूज सही इनपुट ID में फोर्स-फीड हो जाएँ
    setTimeout(() => {
        // संभावित ID लिस्ट जो आपके फॉर्म में हो सकती हैं (ID Mismatch से बचने के लिए दोनों पैटर्न्स चेक किये गए हैं)
        const ghodaInputs = ['vaishno-ghoda-max-member', 'vaishno_ghoda_max_member', 'vaishno-ghoda-max'];
        const palkiInputs = ['vaishno-palki-max-member', 'vaishno_palki_max_member', 'vaishno-palki-max', 'vaishno-dandi-max'];
        const pitthuInputs = ['vaishno-pitthu-max-member', 'vaishno_pitthu_max_member', 'vaishno-pitthu-max'];

        // वैष्णो देवी डेटाबेस की वैल्यूज निकालना
        const gVal = packageData.vaishno_ghoda_max_member || packageData.vaishno_ghoda_max || '';
        const pVal = packageData.vaishno_palki_max_member || packageData.vaishno_palki_max || packageData.vaishno_palki_max_members || '';
        const piVal = packageData.vaishno_pitthu_max_member || packageData.vaishno_pitthu_max || '';

        // फॉर्म में सही ID ढूँढकर वैल्यूज सेट करना (कोई भी ID मैच होने पर खाली नहीं रहेगा)
        ghodaInputs.forEach(id => { const el = document.getElementById(id); if(el) el.value = gVal; });
        palkiInputs.forEach(id => { const el = document.getElementById(id); if(el) el.value = pVal; });
        pitthuInputs.forEach(id => { const el = document.getElementById(id); if(el) el.value = piVal; });
    }, 100);
};
...
window.processSave = async function(pkgId) {
    ...
    try {
        ...
        const pkgData = {
            ...
            // VAISHNO DEVI DATABASE MAPPING (CORRECTED KEY NAME AS PER YOUR DATABASE)
            vaishno_ghoda_price: vaishnoGhodaPrice, 
            vaishno_ghoda_max: vaishnoGhodaMax,
            vaishno_palki_max: vaishnoDandiMax,   // Sahi variable mapping aapki database key ke mutabik
