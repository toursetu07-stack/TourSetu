...
/* =========================================
   6.1 HOTEL ROOM FETCH & BOOKING REQUESTS
   ========================================= */

// Customer aur Agency Dashboard par rooms display karne ke liye
async function loadLiveRoomsForPortals() {
    const client = getClient();
    const { data: rooms, error } = await client
        .from('room_categories')
        .select('*')
        .gt('available_rooms', 0); // Sirf vahi dikhayega jo available hain
    ...
    // Is rooms data ko UI par Render (Card format mein) kar lein
    console.log("Available Rooms:", rooms);
    return rooms;
}

// Room booking request send karne ke liye function
async function sendBookingRequest(hotelId, roomCatId, pricePerNight, totalRoomsNeeded = 1, agencyId = null) {
    const client = getClient();
    const { data: { user } } = await client.auth.getUser();
    ...
    const { data, error } = await client
        ...
        .insert([{
            hotel_id: hotelId,
            room_category_id: roomCatId,
            customer_id: user ? user.id : null,
            agency_id: agencyId, // Agar Agency forward kar rahi hai toh Agency ID pass hogi
            booking_type: agencyId ? 'AGENCY_FORWARD' : 'DIRECT_CUSTOMER',
            rooms_count: totalRoomsNeeded,
            total_amount: pricePerNight * totalRoomsNeeded,
            status: 'PENDING'
        }]);

    if (error) {
        alert("Booking request send nahi ho payi: " + error.message);
    } else {
        alert("Booking Request Sent! Hotel Owner approval ka wait karein.");
    }
}
...
// =========================================
// 7. AGENCY DASHBOARD LOGIC (Ya aapka purana agency code)
// =========================================

function agencyRelatedFunction() {
    // Agency ka code...
} // <-- Agency ka code yahan khatam ho gaya

/* ------------------------------------------------------------- */
/* 👇 ISKHING WAHAN FILE KE BILKUL LAST ME PASTE KAREIN 👇       */
/* ------------------------------------------------------------- */

/* =========================================
   10. HOTEL OWNER DASHBOARD LOGIC
   ========================================= */

// 1. Hotel Owner Dashboard par Incoming Requests Load Karein
async function loadArrivalsAndRequests(hotelId) {
    const client = getClient();
    const { data: requests, error } = await client
        .from('bookings')
        .select('*, room_categories(room_type)')
        .eq('hotel_id', hotelId)
        .order('created_at', { ascending: false });

    if (error) {
        console.error("Error loading arrivals:", error.message);
        return;
    }
...
// 2. Owner Action: Accept (Contact No. ke sath) ya Deny
async function handleOwnerDecision(bookingId, statusChoice) {
    const client = getClient();
    let updateData = { status: statusChoice };

    if (statusChoice === 'ACCEPTED') {
        const phone = prompt("Please enter your Contact Number for Customer/Agency:");
        if (!phone) return alert("Contact number zaroori hai accept karne ke liye!");
        updateData.owner_contact = phone;
    }
    ...
    if (!error) {
        alert(`Booking status changed to ${statusChoice}!`);
    } else {
        alert("Error updating status: " + error.message);
    }
}
// 3. UI par incoming requests ko dynamically render/dikhane ke liye function
async function fetchAndRenderOwnerRequests(hotelId) {
    const container = document.getElementById('owner-requests-list');
    if(!container) return;
    
    container.innerHTML = "<p>Loading requests...</p>";

    // Pehle se likha hua function call karega
    const requests = await loadArrivalsAndRequests(hotelId);

    if (!requests || requests.length === 0) {
        container.innerHTML = "<p>No incoming booking requests found.</p>";
        return;
    }

    // UI Clear karke naya data dikhayega
    container.innerHTML = '';

    requests.forEach(req => {
        const roomType = req.room_categories ? req.room_categories.room_type : 'N/A';
        ...
        card.innerHTML = `
            <p><strong>Booking ID:</strong> ${req.id}</p>
            <p><strong>Room Category:</strong> ${roomType}</p>
            <p><strong>Rooms Count:</strong> ${req.rooms_count}</p>
            <p><strong>Total Amount:</strong> ₹${req.total_amount}</p>
            <p><strong>Status:</strong> <b style="color: ${req.status === 'PENDING' ? 'orange' : req.status === 'ACCEPTED' ? 'green' : 'red'}">${req.status}</b></p>
            ${req.owner_contact ? `<p><strong>Your Contact Shared:</strong> ${req.owner_contact}</p>` : ''}
            
            ${req.status === 'PENDING' ? `
                <button onclick="handleOwnerDecision('${req.id}', 'ACCEPTED')" style="background: green; color: white; padding: 5px 10px; margin-right: 5px;">Accept Request</button>
                <button onclick="handleOwnerDecision('${req.id}', 'REJECTED')" style="background: red; color: white; padding: 5px 10px;">Reject Request</button>
